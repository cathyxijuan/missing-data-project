...
#Data with two missing variables 
...
df04 <- fitNoMissingShort_CR1_1[[1]][1:5,]
df204 <- fitMCAR_Short_CR1_1[[5]][1:5,]
df504 <- fitMCAR_Short_CR1_1[[6]][1:5,]



sv04 <- fitNoMissingShort_CR1_3[[1]][1:5,]
sv204 <- fitMCAR_Short_CR1_3[[5]][1:5,]
sv504 <- fitMCAR_Short_CR1_3[[6]][1:5,]
...
ggplot(datafinal, aes(x=Model, y=Fmin, colour=PercentMissing)) + geom_line(aes(group=PercentMissing)) + 
  geom_point()+facet_grid(PlaceMiss~NumMissVar)+ scale_colour_discrete(name="Percent Missing")
