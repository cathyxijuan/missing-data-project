...
#purpose: create missing data on x1,  x7.
#Argument:
...
MCARMinPattern <- function(model, sample.nobs=1000000,  missing.percentage){
  ...
  simuData[1:(sample.nobs*missing.percentage),  c(1:2, 7:8)] <-NA
  simuData
}

#Usage: only for this research. There are six variables with missing data; maximum number of missing patterns for three variables: (1,7), (2,8) and (3, 9)
#Argument:
#model: lavaan defined population model
#sample.nobs: numeric; sample size without missing data
#missing.percentage: numeric; a proportion of missing data
...
#Usage: only for this research. There are six variables with missing data; maximum number of missing patterns for three variables: (3,7), (4,8) and (5, 9)
#Argument:
#model: lavaan defined population model
#sample.nobs: numeric; sample size without missing data
#missing.percentage: numeric; a proportion of missing data
...
MCARMaxPattern2 <- function(model, sample.nobs=1000000,  missing.percentage){
  ...
  for(i in 0:2){
    simuData[1:(sample.nobs*missing.percentage/(2^i)),c(1+i, 7+i)] <- NA
  }
  simuData
}
